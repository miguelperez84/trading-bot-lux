...
//CALCULAR BOILER
  function calcularBollingerBands(cierres, periodo = 20, desviacion = 2) {
    ...
    for (let i = periodo - 1; i < cierres.length; i++) {
      ...
      const varianza = slice.reduce((acc, val) => acc + Math.pow(val - promedio, 2), 0) / periodo;
  ...
  async function obtenerDatos() {
    try {
      ...
      const response = await fetch(`https://api.binance.com/api/v3/klines?symbol=${simbolo}&interval=${intervaloVelas}&limit=${limit}`);
...
async function actualizarGrafico() {
  ...
  const datosEMA12 = etiquetas.map((x, i) => ({ x, y: ema12[i + (ema12.length - etiquetas.length)] }));
    const datosEMA26 = etiquetas.map((x, i) => ({ x, y: ema26[i + (ema26.length - etiquetas.length)] }));
  ...
  if (!rsiChart) {
    rsiChart = new Chart(ctxRSI, {
      type: 'line',
      data: { labels: rsi.map((_, i) => i), datasets: [{ label: 'RSI', data: rsi, borderColor: 'blue' }] }
    });
  } else {
  ...
  if (!macdChart) {
    macdChart = new Chart(ctxMACD, {
      ...
      data: {
        ...
        datasets: [
          { type: 'line', label: 'MACD', data: macdLine, borderColor: 'green', borderWidth: 2 },
          { type: 'line', label: 'Signal', data: signalLine, borderColor: 'red', borderWidth: 2 },
          { type: 'bar', label: 'Histograma', data: histograma, backgroundColor: histograma.map(v => v >= 0 ? 'rgba(0,200,0,0.5)' : 'rgba(200,0,0,0.5)') }
        ]
      }
    });
  } else {
...
const coloresADX = adxValues.map(v => v > 25 ? 'rgba(0,200,0,0.7)' : 'rgba(150,150,150,0.5)');
...
// INICIACION DE ACTUALIZACIONES
  ...
  selectorMoneda.addEventListener('change', () => {
    ...
    chart?.destroy(); rsiChart?.destroy(); macdChart?.destroy();
